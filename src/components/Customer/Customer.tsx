...
export const Customer = () => {
  ...
  const [currentCustomer, setCurrentCustomer] = useState<CustomerModel | null>(
    new CustomerModel("", 0, "", 0)
  );
  const [isShowUpdateInforModal, setIsShowUpdateInforModal] =
    useState<boolean>(false);
  const [isShowInputInforModal, setIsShowInputInforModal] =
    useState<boolean>(false);
  ...
  const handleOnClickExportCustomer = (
  ...
  const updateCustomer = async () => {
    const url: string = "http://localhost:8081/customers";
    const response = await axiosPrivate.put(url, currentCustomer);
    loadAllCustomer();
  ...
  const createCustomer = async (customer: CustomerModel) => {
    const url: string = "http://localhost:8081/customers";
    const response = await axiosPrivate.post(url, customer);
    loadAllCustomer();
  };
    ...
    <>
      ...
      {currentCustomer && (
        <UpdateCustomerModal
          ...
          setCurrentCustomer={setCurrentCustomer}
          isShowUpdateCustomerModal={isShowUpdateInforModal}
          setIsShowUpdateCustomerModal={setIsShowUpdateInforModal}
          updateCustomer={updateCustomer}
        />
      )}
      <CreateCusModal
        isShowInputInforModal={isShowInputInforModal}
        setIsShowInputInforModal={setIsShowInputInforModal}
      ...
      >
        <button
          ...
          onClick={handleOnClickExportCustomer}
          id="btnExportCustomer"
        >
